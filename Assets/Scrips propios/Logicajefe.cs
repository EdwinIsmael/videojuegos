...
public class Logicajefe : MonoBehaviour {
    ...
    void RevisarVida()
    {
        ...
        if(vida.valor <= 0)
        {
            sumarPuntos = true;
            if (sumarPuntos){
                puntajePantalla.GetComponent<Puntaje>().valor += 15;
                sumarPuntos=false;
            }
            Vida0 = true;
            agente.isStopped = true;
            collider.enabled = false;
            animator.CrossFadeInFixedTime("Vida0", 0.5f);
    ...
    void RevisarAtaque()
    {
        ...
        if(distanciaDelBlanco <= 1f && mirando)
        {
            Atacar();
        }
    }
